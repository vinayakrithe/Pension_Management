...
namespace ProcessPensionMicroservice.Repository
{
    public class ProcessPensionRepository : IProcessPensionRepository
    {
        ...
        public async Task<PensionerDetail> GetPensionerDetailAsync(string aadharNumber,string token)
        {
            ...
            {
                using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, "https://localhost:44338/api/PensionerDetails/GetPensionerDetail/" + aadharNumber))
                {
        ...
        public async Task SaveDataAsync(string token)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, "https://localhost:44338/api/PensionerDetails/GetDetails"))
                {
                    requestMessage.Headers.Authorization =
                    new AuthenticationHeaderValue("Bearer", token);
